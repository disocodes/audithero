...
def test_roster_explicit_match():
    ts=pd.DataFrame([{'timesheet_id':'T1','employee_id':'E1','rostered_shift_id':'R1','start_datetime':'2026-08-03 08:00','end_datetime':'2026-08-03 18:00'}]);r=normalize_rosters([{'id':'R1','member_id':'E1','start_time':'2026-08-03 09:00','end_time':'2026-08-03 17:00','breaks':[]}]);o=attach_rosters(ts,r);assert o.iloc[0].rostered_shift_match_status=='EXPLICIT_ID'
def test_full_time_rostered_overtime():
    ts=pd.DataFrame([{'timesheet_id':'T1','employee_id':'E1','start_datetime':'2026-08-03 08:00','end_datetime':'2026-08-03 18:00','rostered_start_datetime':'2026-08-03 09:00','rostered_end_datetime':'2026-08-03 17:00','rostered_shift_match_status':'EXPLICIT_ID'}]);d=pd.DataFrame([{'timesheet_id':'T1','employee_id':'E1','employee_name':'X','employment_type':'FULL_TIME','classification_code':'SACS-L2-P3','work_group':'DISABILITY_SERVICES','state':'WA','award_reference_date':'2026-08-03','shift_start':'2026-08-03 08:00','shift_end':'2026-08-03 18:00','worked_hours':10.0,'base_hourly_rate':38.50,'expected_amount':385.00,'entitlement_status':'CALCULATED','review_flags':'','calculation_evidence':'[]','pay_period_start':'2026-08-03','pay_period_end':'2026-08-16'}]);o=apply_rostered_and_daily_overtime(d,ts,holidays(),LIB);assert o.iloc[0].expected_amount==423.50
def test_casual_daily_overtime():
    ts=pd.DataFrame([{'timesheet_id':'T1','employee_id':'E1','start_datetime':'2026-08-03 07:00','end_datetime':'2026-08-03 18:00','break_units':0}]);d=pd.DataFrame([{'timesheet_id':'T1','employee_id':'E1','employment_type':'CASUAL','classification_code':'SACS-L2-P3','work_group':'DISABILITY_SERVICES','state':'WA','award_reference_date':'2026-08-03','shift_start':'2026-08-03 07:00','shift_end':'2026-08-03 18:00','worked_hours':11.0,'base_hourly_rate':38.50,'expected_amount':529.43,'entitlement_status':'CALCULATED','review_flags':'','calculation_evidence':'[]','pay_period_start':'2026-08-03','pay_period_end':'2026-08-16'}]);o=apply_rostered_and_daily_overtime(d,ts,holidays(),LIB);assert o.iloc[0].expected_amount==548.68
def context():
    e=pd.DataFrame([{'employee_id':'E1','employee_name':'X','employment_type_current':'CASUAL','state':'WA','work_group':'DISABILITY_SERVICES'}]);c=pd.DataFrame([{'employee_id':'E1','effective_from':'2020-01-01','classification_code':'SACS-L2-P3'}]);return e,c
