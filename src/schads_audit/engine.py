...
def calculate_entitlements(employees,employment_history,classifications,timesheets,holidays,lib):
    ...
    for _,ts0 in timesheets.iterrows():
        ...
        pp_start=_dt(ts.get('pay_period_start'));reference=pp_start or start
        ...
        segs=_split(start,end,float(ts.get('break_units') or 0)*60);worked=sum(s['hours'] for s in segs);stype=_shift_type(start,end);expected=Decimal('0');evidence=[]
        if base is not None and conditions and emp_type in ('FULL_TIME','PART_TIME','CASUAL'):
            ...
            if worked>10 and 'PUBLIC_HOLIDAY' not in {_day_type(s['date'],state,holidays) for s in segs}:
                flags.append('DAILY_OVERTIME_REVIEW')
            ...
            if worked>float(conditions['meal_break']['review_if_shift_gt_hours_and_no_break']) and not float(ts.get('break_units') or 0):flags.append('MEAL_BREAK_REVIEW')
