...
def _invalid_datetime_mask(series: pd.Series) -> pd.Series:
    return (~_blank_mask(series)) & parse_datetime_series(series).isna()
...
def assess_file_readiness(input_root,config_root):
    ...
    for name,required in REQUIRED_COLUMNS.items():
        frame=frames[name];missing=sorted(required-set(frame.columns));add("INPUT_COLUMNS",name,"BLOCKING" if missing else "READY",("Missing columns: "+", ".join(missing)) if missing else "Required columns present")
        if missing:continue
        blank_fields=[]
        for field in sorted(required):
            count=int(_blank_mask(frame[field]).sum())
            if count:blank_fields.append(f"{field} ({count})")
        if blank_fields:add("INPUT_REQUIRED_VALUES",name,"BLOCKING","Blank required value(s): "+", ".join(blank_fields))
        else:add("INPUT_REQUIRED_VALUES",name,"READY","Required values populated")
    ...
    for dataset,frame in frames.items():
        ...
        for column in frame.columns:
            if not str(column).startswith("_invalid_"):continue
            count=int(frame[column].fillna(False).astype(bool).sum())
            if count:
                field=str(column)[len("_invalid_"):];status="BLOCKING" if field in REQUIRED_COLUMNS.get(dataset,set()) else "REVIEW"
                add("INPUT_VALUE_TYPE",f"{dataset}.{field}",status,f"{count} non-blank value(s) could not be converted to the canonical type")
    temporal_fields={"pay_details":["effective_from"],"employment_history":["start_date"],"timesheets":["start_datetime","end_datetime"]}
    for dataset,fields in temporal_fields.items():
        frame=frames[dataset]
        for field in fields:
            if field in frame.columns:
                invalid=int(_invalid_datetime_mask(frame[field]).sum())
                if invalid:add("INPUT_DATETIME",f"{dataset}.{field}","BLOCKING",f"{invalid} populated value(s) are not valid datetimes")
    ts=frames["timesheets"]
    if {"start_datetime","end_datetime"}.issubset(ts.columns):
        starts=parse_datetime_series(ts["start_datetime"]);ends=parse_datetime_series(ts["end_datetime"]);invalid_order=int((starts.notna()&ends.notna()&(ends<=starts)).sum())
        if invalid_order:add("SHIFT_INTERVAL","timesheets","BLOCKING",f"{invalid_order} shift(s) have end_datetime less than or equal to start_datetime")
    ...
    if "employee_id" in frames["employees"].columns:
        ...
        if dup:add("KEY_UNIQUENESS","employees.employee_id","BLOCKING",f"{dup} employee row(s) use duplicated employee_id values")
    if "timesheet_id" in ts.columns:
        dup=int(ts["timesheet_id"].astype(str).str.strip().duplicated(keep=False).sum())
        if dup:add("KEY_UNIQUENESS","timesheets.timesheet_id","BLOCKING",f"{dup} timesheet row(s) use duplicated timesheet_id values")
    for dataset in ("pay_details","employment_history","timesheets"):
        frame=frames[dataset]
        if "employee_id" not in frame.columns:continue
        refs=set(frame["employee_id"].dropna().astype(str).str.strip());unknown=sorted(x for x in refs if x and x not in employee_ids)
        if unknown:
            sample=", ".join(unknown[:10]);suffix=" ..." if len(unknown)>10 else "";add("REFERENTIAL_INTEGRITY",f"{dataset}.employee_id","BLOCKING",f"{len(unknown)} employee ID(s) are not present in employees: {sample}{suffix}")
    ...
    pay_runs=frames.get("pay_runs",pd.DataFrame());missing_period="pay_period_start" not in ts.columns or ts["pay_period_start"].isna().any()
    ...
    if not _control_present(frames,root,"industrial_instrument_history"):add("CONTROL_REGISTER","industrial_instrument_history","BLOCKING","Historical remediation requires Award/EA/IFA coverage evidence; add the workbook sheet or sidecar CSV")
    ...
    if has_pt and not _control_present(frames,root,"part_time_patterns"):add("CONTROL_REGISTER","part_time_patterns","BLOCKING","Part-time employees detected; add effective-dated written pattern history")
    ...
    if payroll.empty:add("ACTUAL_PAY","payroll_earnings","REVIEW","Expected entitlements can be calculated; actual-pay reconciliation is unavailable because payroll earnings were not supplied")
    else:
        needed={"employee_id","pay_period_start","pay_period_end","pay_category","amount"};miss=sorted(needed-set(payroll.columns));invalid_required=[field for field in needed if field in payroll.columns and _blank_mask(payroll[field]).any()];unknown_payroll=[]
        if "employee_id" in payroll.columns:
            refs=set(payroll["employee_id"].dropna().astype(str).str.strip());unknown_payroll=sorted(x for x in refs if x and x not in employee_ids)
        if miss or invalid_required or unknown_payroll:
            ...
            add("ACTUAL_PAY","payroll_earnings","READY","Actual payroll earnings available");mapping=frames.get("pay_category_mapping",pd.DataFrame());sidecar=(root/"pay_category_mapping.json").exists()
            if (mapping is None or mapping.empty) and not sidecar:add("ACTUAL_PAY","pay_category_mapping","REVIEW","Payroll earnings are available but no pay-category treatment mapping exists; expected entitlements can still be calculated")
