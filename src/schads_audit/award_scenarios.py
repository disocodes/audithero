        ...
        rows.append(
            {
                **base,
                ...
                "day_type": None,
                "shift_type": None,
                "detail": "Award minimum base hourly rate for the selected classification scenario.",
            }
        )
        try:
...
def calculate_award_scenarios(
    ...
    *,
    ...
    Scenario calculation is deliberately independent of the employee's configured
    classification. Each supplied shift is recalculated across the selected SCHADS
    classifications and employment types so an auditor can explore the Award result
    even when employee master data is incomplete. Definitive audit status remains a
    separate concept and is not manufactured from a scenario selection.
    """
        ...
        for emp_type in employment_types:
                ...
                [
                    {
                        ...
                        "effective_from": _first_shift(timesheets, row["employee_id"]) or earliest,
                        "classification_code": code,
            ...
            synthetic_history = pd.DataFrame(
                [
                    {
                        "employee_id": str(row["employee_id"]),
                        "start_date": _first_shift(timesheets, row["employee_id"]) or earliest,
                        "end_date": pd.NaT,
            ...
            meta = {
                ...
                "classification_family": cls.get("classification_family"),
