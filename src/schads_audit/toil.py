...
def audit_toil_register(register,employees,classifications,holidays,lib,audit_end_date=None):
    ...
    for _,r0 in register.iterrows():
        ...
        payment_due=False;reason=None
        if remaining>0 and payment_request is not None:payment_due=True;reason='EMPLOYEE_REQUEST'
        elif remaining>0 and termination is not None:payment_due=True;reason='TERMINATION'
        elif remaining>0 and due_date is not None and audit_end>=due_date:payment_due=True;reason='THREE_MONTH_EXPIRY';payment_date=payment_date or due_date
        expected=Decimal('0')
        if payment_due:
            ...
            if not code and classifications is not None and not classifications.empty and worked is not None:
                q=classifications[classifications['employee_id'].astype(str)==eid].copy();q['_s']=parse_datetime_series(q['effective_from']);q=q[q['_s']<=worked]
                if not q.empty:code=q.sort_values('_s',ascending=False).iloc[0].get('classification_code')
            if not code or payment_date is None or worked is None:flags.append('TOIL_PAYMENT_RATE_FACTS_MISSING')
            else:
        ...
        elif remaining>0 and due_date is not None:evidence.append({'component':'TOIL_OUTSTANDING','remaining_hours':remaining,'deadline':str(due_date),'clause':'28.2(d)'})
