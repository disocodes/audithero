...
def _apply_timesheet_mappings(df,work_types,locations):
    ...
    for i,r in out.iterrows():
        for k in [str(r.get('work_type_id') or ''),str(r.get('work_type_name') or '')]:
            if k in work_types:
                v=work_types[k]
                if isinstance(v,dict):
                    for f in ('work_group','is_sleepover','broken_shift_breaks'):
                        if f in v:out.at[i,f]=v[f]
                break
        k=str(r.get('work_site_id') or '')
        if k in locations:
...
def run_databricks_audit(spark,dbutils,config,lib,start_date,end_date,mapping_dir,run_type='HISTORICAL',actual_pay_source=None):
    ...
    try:
        ...
        employees=apply_employee_overrides(normalize_employees(employees_raw),load_json(mapping_dir/'employee_overrides.json',{}));pay_details=apply_classification_mapping(normalize_pay_details(pay_raw),load_json(mapping_dir/'classification_mapping.json',{}));emp_hist=normalize_employment_history(emp_hist_raw);timesheets=_apply_timesheet_mappings(normalize_timesheets(ts_raw),load_json(mapping_dir/'work_type_mapping.json',{}),load_json(mapping_dir/'work_location_state_mapping.json',{}))
        ...
        detail=calculate_entitlements(employees,emp_hist,pay_details,timesheets,holidays,lib);recon=reconcile_pay_periods(detail,payroll,load_json(mapping_dir/'pay_category_mapping.json',{}),config.variance_tolerance)
        ...
        for name,df in [('employees',employees),('pay_details',pay_details),('employment_history',emp_hist),('timesheets',timesheets),('payroll_earnings',payroll),('public_holidays',holidays)]:
            if not df.empty:
