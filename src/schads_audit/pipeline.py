...
def _apply_timesheet_mappings(df,work_types,locations):
    ...
    for i,r in out.iterrows():
        ...
        for k in [str(r.get('work_site_id') or ''),str(r.get('location_id') or '')]:
            if k in locations:
                v=locations[k];out.at[i,'location_state']=v.get('state') if isinstance(v,dict) else v
                if isinstance(v,dict) and v.get('holiday_location_key'):out.at[i,'holiday_location_key']=v.get('holiday_location_key')
                break
    return out
...
def run_databricks_audit(spark,dbutils,config,lib,start_date,end_date,mapping_dir,run_type='HISTORICAL',actual_pay_source=None):
    ...
    try:
        ...
        timesheets=_apply_timesheet_mappings(normalize_timesheets(ts_raw),load_json(mapping_dir/'work_type_mapping.json',{}),load_json(mapping_dir/'work_location_state_mapping.json',{}));timesheets=group_broken_shifts(timesheets);timesheets=group_sleepovers(timesheets);timesheets=attach_rosters(timesheets,rosters)
