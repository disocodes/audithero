...
def normalize_timesheets(items):
    ...
    for x in items:
        ...
        rows.append({'timesheet_id':first(x,'id','timesheet_entry_id','timesheetEntryId','Id'),'employee_id':first(x,'employee_id','employeeId','member_id','memberId'),'start_datetime':s,'end_datetime':e,'units':first(x,'units','hours','total_hours','totalHours'),'break_units':first(x,'break_units','breakUnits'),'work_type_id':first(x,'work_type_id','workTypeId'),'work_type_name':first(x,'work_type','workType','work_type_name','workTypeName'),'work_site_id':first(x,'work_site_id','workSiteId','work_location_id','workLocationId'),'comment':first(x,'comment','notes','description'),'status':first(x,'status'),'raw_json':raw_json(x)})
...
def apply_classification_mapping(df,mapping):
    out=df.copy();out['classification_code']=out.apply(lambda r: next((mapping[k].get('classification_code') if isinstance(mapping[k],dict) else mapping[k] for k in [str(r.get('pay_detail_id') or ''),str(r.get('classification_name') or '')] if k in mapping),infer_schads_classification(r.get('classification_name'))),axis=1);return out
def apply_employee_overrides(df,overrides):
    out=df.copy();out['work_group']='DISABILITY_SERVICES'
