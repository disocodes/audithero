...
def draft_to_file_context_table(draft: dict[str, Any]) -> pd.DataFrame:
    ...
    for item in _inventory(draft):
        ...
        rows.append({"source_file":item["file"],"source_sheet":item["sheet"],"detected_role":role,"detected_confidence":confidence,"selected_role":role or "CONTEXT_ONLY","use_as_primary_source":bool(role),"evidence_purpose":"","period_start":"","period_end":"","document_status":"","notes":""})
    return pd.DataFrame(rows)
...
def draft_to_preview_table(draft: dict[str, Any]) -> pd.DataFrame:
    rows=[]
    for dataset,schema in CANONICAL_SCHEMAS.items():
        cfg=(draft.get("datasets") or {}).get(dataset) or {}; source=cfg.get("source") or {}; mapped=cfg.get("columns") or {}; required=list(schema.get("required", [])); missing=[f for f in required if f not in mapped]
        rows.append({"dataset":dataset,"detected":bool(cfg.get("enabled",False)),"source_file":source.get("file"),"source_sheet":source.get("sheet"),"confidence":cfg.get("_dataset_confidence"),"mapped_fields":len(mapped),"required_fields":len(required),"missing_required_fields":", ".join(missing),"preview_status":"READY_TO_REVIEW" if cfg.get("enabled") and not missing else ("MAPPING_INCOMPLETE" if cfg.get("enabled") else "NOT_DETECTED")})
    return pd.DataFrame(rows)
...
def write_mapping_workbook(draft:dict[str,Any],path:str|Path)->Path:
    """Write an optional controlled Advanced Mapping correction/context workbook."""
    ...
    wb=load_workbook(path); lists_ws=wb.create_sheet("_lists"); lists_ws.sheet_state="hidden"; lists=_excel_lists(draft)
    ws=wb["field_mapping"]; ws.freeze_panes="A2"; ws.auto_filter.ref=ws.dimensions
    ...
    _add_range_validation(wb,vm,"A2:A1000",lists["datasets"],"value_datasets"); _add_range_validation(wb,vm,"B2:B1000",lists["fields"],"value_target_fields")
    readme=wb["README"]; readme.column_dimensions["A"].width=30; readme.column_dimensions["B"].width=118; wb.save(path); return path
...
def load_mapping_workbook(path:str|Path)->dict[str,Any]:
    ...
    for dataset,group in fields.groupby("dataset",dropna=True):
        ...
        for _,row in group.iterrows():
            ...
            elif _text(row.get("date_source")) and _text(row.get("time_source")):rule["date_source"]=_text(row.get("date_source"));rule["time_source"]=_text(row.get("time_source"))
            elif _text(row.get("constant")) is not None:rule["constant"]=row.get("constant")
            else:continue
            dtype=_text(row.get("data_type"))
            if dtype:rule["type"]=dtype
